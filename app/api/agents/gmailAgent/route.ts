...
export async function GET() {
  return NextResponse.json({
    message: 'Gmail Task Assignment Agent',
    description: 'Analyzes team members and sends personalized task assignment emails to relevant users',
    usage: {
      method: 'POST',
      endpoint: '/api/agents/gmailAgent',
      contentType: 'application/json',
      authentication: 'Required - Auth0 session',
      requirements: [
        'User must have connected Google account',
        'Google account must have Gmail send permissions'
      ],
      body: {
        users: [
          {
            ...
            description: 'Responsible for React components and UI implementation'
          }
        ],
        solutionPlan: 'Implement a new user dashboard with real-time analytics and customizable widgets'
      }
    },
    workflow: [
      '1. AI analyzes which users are relevant based on their role and description',
      '2. Generates personalized emails for each relevant user',
      '3. Sends emails via Gmail API',
      '4. Returns summary of results'
    ],
    outputFormat: {
      totalUsers: 'Total number of users provided',
      relevantUsers: 'Number of users identified as relevant',
      emailsSent: 'Number of emails successfully sent',
      results: 'Array of results for each email attempt',
